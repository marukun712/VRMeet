...
export const startMediaPipeTracking = (
  ...
  const onResults = (results: Results) => {
    if (videoElement == null || myVRM == null) {
      return;
    } //自分のVRMモデルがロード済みなら
    if (loading) {
      finishLoading();
  ...
  const startCamera = async () => {
    ...
    promise
      ...
      .catch((error) => {
        alert(
          "カメラ映像の取得に失敗しました。カメラの使用権限を付与して再度参加してください。"
        );
        location.href = "/";
      });
  };
